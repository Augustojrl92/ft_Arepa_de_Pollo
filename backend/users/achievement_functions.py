...
def set_up_achievements():
	...
	create_achievement_if_unexistant(coalition_rank_name, 'Sé el primero de tu coalición', 1)

	private_achievement_name = 'Es tu primer día?'
	Achievement.completion_check_funcs[private_achievement_name] = private_completion
	create_achievement_if_unexistant(private_achievement_name, 'Unete a AEDLPH', 1)

	Achievement.completion_check_funcs['Es tu primer día, eh?'] = private_completion

def create_achievement_if_unexistant(name: str, description: str, completion_points: int, experience: int, daily: bool, icon_HTML: str | None = None) -> None:
